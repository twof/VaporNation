...
You're now able to have routes be called, even if the case of the requested URL isn't quite right, which means that
...
This fix should resolve a couple of MySQL bugs.
...
There were a few deprecated options that made the toolbox unusable for a bit, but all is good now again!
