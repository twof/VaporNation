...
```
You can use anything that conforms to `PathComponentsRepresentable` to create the path, including type constrained paramameters.
...
- Also the RW's Vapor [Server Side Swift with Vapor](https://videos.raywenderlich.com/courses/115-server-side-swift-with-vapor/lessons/1)
