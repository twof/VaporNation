...
### SQLite Models No Longer Require `init` Methods

This is true for both `struct`- and `class`-based models.
...
It is now possible to provide a pre-configured `URLSession` when using `FoundationClient`. This especially allows proxying via HTTPS, along with other parameters configurable via `URLSessionConfiguration`.
